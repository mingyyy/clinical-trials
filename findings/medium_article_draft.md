...
Against the corrected ground truth, Fix D reached 75.8% overall accuracy across four patients (94 of 124 comparable assessments; one patient excluded because the API returned a different trial set on different run days).

The error breakdown is instructive. Three verdicts were ELIGIBLE when the correct answer was INELIGIBLE — the highest-severity failure. In all three cases, Fix D's predicate extractor missed an inclusion requirement: "advanced/metastatic solid tumor required" and "HER2-overexpressing excluded" were not captured as predicates, so code found no failing check and returned ELIGIBLE. Eight verdicts were INELIGIBLE when the correct answer was UNCERTAIN — Fix D treating a null field as a failed predicate rather than triggering DATA_MISSING. Eighteen verdicts were UNCERTAIN when the correct answer was INELIGIBLE — the predicate ontology missing "active/measurable disease required" as a hard inclusion gate.

The dominant error mode — UNCERTAIN when the correct answer is INELIGIBLE — is clinically the safer failure. A false UNCERTAIN sends a case to human review. A false INELIGIBLE removes a potentially eligible patient from consideration silently. Fix D errs toward caution.

The three problems (missed predicates, null-as-fail, missing active-disease gate) are ontology coverage gaps and one implementation error — not architectural flaws. The work has moved downstream from prompt calibration to ontology maintenance. That is a different kind of engineering problem, but it is a tractable one. And the verification exercise itself surfaced a methodological finding: even careful human labeling makes systematic errors when the reviewer applies "absent = uncertain" to cases where the profile information is explicitly present.
