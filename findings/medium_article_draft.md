...
To measure Fix D's accuracy, I hand-labeled ground truth for all five patients by reading actual ClinicalTrials.gov eligibility criteria independently of any framework output. Each trial received one of three labels: eligible, ineligible, or ambiguous (= UNCERTAIN). Against those labels, Fix D reached 77.4% overall accuracy across four patients (96 of 124 comparable assessments; one patient excluded because the API returned a different trial set on different run days).

The error breakdown is instructive. Three verdicts were ELIGIBLE when the correct answer was INELIGIBLE — the highest-severity failure, because it could send a patient to a trial they cannot join. In all three cases, Fix D's predicate extractor missed an inclusion requirement: "advanced/metastatic solid tumor required" and "HER2-overexpressing excluded" were not captured as predicates, so code found no failing check and returned ELIGIBLE. Thirteen verdicts were INELIGIBLE when the correct answer was UNCERTAIN — the inverse of the original confidence problem. Fix D treated a null field as a failed predicate rather than triggering DATA_MISSING. Eleven verdicts were UNCERTAIN when the correct answer was INELIGIBLE: the predicate ontology didn't reliably encode "active/measurable disease required" as a hard inclusion gate, so patients with no evidence of disease received false ambiguity flags.

The first problem (missed predicates) is a coverage gap in the ontology and extraction prompt — fixable by expanding the predicate vocabulary. The second problem (null = fail instead of null = unknown) is an implementation error in the evaluation code. The third problem (missing "active disease" gate) is an ontology gap. None of these are fundamental architectural flaws. All three point to the same conclusion: the architecture is correct, and the work has moved downstream — from prompt calibration to ontology maintenance. That is a different kind of engineering problem, but it is a tractable one.
